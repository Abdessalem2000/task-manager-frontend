...
export default function WorkingApp() {
  ...
  useEffect(() => {
    if (isLoaded && mounted) {
      ...
      setCompletedToday(2); // 2 tasks completed from demo data
  ...
  // Fetch data from APIs only if user is signed in
  useEffect(() => {
            ...
            <div>
              ...
              {user && (
                <p style={{ 
                  ...
                  Welcome back, {user.firstName || user.email?.split('@')[0]}! Here's your productivity overview.
                </p>
              )}
              {!user && (
                <p style={{ 
